function Register_2D_Zstack_2P_v2 
...
comp=1 %1 is in vivo vs processed; 1.1 is invivo vs processed, processed is fixed; 2 is procesessd, GRIN vs tissue side; ; 2.1 is procesessd, GRIN vs tissue side, 1100nm;3 is confocal to 2P, tissue side; 4 is invivo to 2P
i=1; % index of plane 
...
%     case 2.1
%       fullpath=['D:\DATA_Glab\Light_sectioning\' mouse '\fit_files\' M_num '_processed1100nm_' num2str(Z) '_FIT_' method_fixed '.mat' ];
    case 3
        fullpath=[path mouse '\fit_files\' M_num '_2PtoConfocal_' num2str(Z) '_FIT_' method_fixed '.mat' ];
    case 4
        fullpath=[path mouse '\fit_files\' M_num '_2PtoInvivo_' num2str(Z) '_FIT_' method_fixed '.mat' ];
    case 5
        fullpath=[path mouse '\fit_files\' M_num '_proceesed1100nm_' num2str(Z) '_FIT_' method_fixed '.mat' ];
end
...
atpath{2}=[TXT{this_mouse_ind+2,2} '\s-' num2str(Z) 'um_v2_nrMC_mean.tif']; 
...
switch comp
    ...
    case 2 % procesessd, GRIN vs tissue side,940nm   
     ...
     case 4 % procesessd, tissue side, 2P vs invivo

        % load processed GRIN imaging data
        disp('load ''processed Tissue, 2 photon'' data')
        movingImage=imread([atpath{3} 'section_'  num2str(choose_section3) '.tif']);
        movingImage=fliplr(movingImage);
        
        % load invivo data
        disp('load ''in vivo'' data')
        fixedImage=imread(atpath{2});
    case 5 % procesessd, tissue side, 940nm vs 1100nm - small adjustment is needed - did it with imageJ instead

        % load processed GRIN imaging data
        disp('load ''processed Tissue, 940nm'' data')
        fixedImage=imread([atpath{3} 'section_8.tif']);
        fixedImage=fliplr(fixedImage);
       % extra one
        
        % load invivo data
        disp('load ''processed Tissue, 1100nm '' data')
        movingImage=imread([atpath{5} 'section_8.tif']);
        movingImage=fliplr(movingImage);
end
% 
...
BrF(1)=3;
BrF(2)=30;
[AmovingImage, AfixedImage]=adjust_brightness(movingImage,fixedImage,BrF(1),BrF(2),1);
...
subplot (1,4,4), imshowpair(AfixedImage,Jregistered), title('fixed and registered Image')
subplot (1,4,2), imshowpair(fake1,Jregistered), title('registered moving Image')
...
switch comp
    ...
    case 3
        text(-350,600,[ 'section ' num2str(choose_section4) ',to section' num2str(choose_section3) ', ' mouse_title, ', ' method_fixed])
    case 4
        text(-350,600,[ num2str(Z) 'um  ,to section' num2str(choose_section3) ', ' mouse_title, ', ' method_fixed])
end

figure, imshowpair(AfixedImage,Jregistered), title('fixed image and registered Image')
...
switch comp
    case 1
      %  save(['D:\DATA_Glab\Light_sectioning\' mouse '\fit_files\' M_num '_invivo1_' num2str(Z) '_tform_NRS'],'mytform')
        ...
        imwrite(Jregistered,[atpath{2}(1:sind(end))   num2str(Z)  'nm_registered_GRIN2invivo.tif'])
          if ~exist([path mouse '\'   num2str(Z) 'um\']); mkdir([path mouse '\'   num2str(Z) 'um\']); end
          cd ([path mouse '\'   num2str(Z) 'um\']); 
          imwrite(Jregistered,[num2str(Z)  'nm_registered_GRIN2invivo.tif'])
    case 2
      %  save(['D:\DATA_Glab\Light_sectioning\' mouse '\fit_files\' M_num '_processed_940nm_' num2str(Z) '_tform_' method_fixed ],'mytform')
        ...
        imwrite(AfixedImage,[path mouse '\'   num2str(Z) 'um\' 'section_' num2str(choose_section2) '_Tissue2GRIN940nm.tif'])
    case 3 % the matrix saved is in relate to **adjusted** tissue, 2P!!!!  
     %   save(['D:\DATA_Glab\Light_sectioning\' mouse '\fit_files\' M_num '_2PtoConfocal_' num2str(Z) '_tform_' method_fixed ],'mytform')
        save(['D:\DATA_Glab\Light_sectioning\' mouse '\fit_files\' M_num '_2PtoConfocal_' num2str(Z) '_FIT_' method_fixed ],'FIT')
        imwrite(Jregistered,[atpath{4} 'section_registered_' num2str(choose_section4-1) '.tif'])
  
  case 4 % the matrix saved is in relate to 2P 
      %  save(['D:\DATA_Glab\Light_sectioning\' mouse '\fit_files\' M_num '_2Ptoinvivo_' num2str(Z) '_tform_' method_fixed ],'mytform')
        save(['D:\DATA_Glab\Light_sectioning\' mouse '\fit_files\' M_num '_2Ptoinvivo_' num2str(Z) '_FIT_' method_fixed ],'FIT')
        sind=strfind(atpath{2},'\');
        imwrite(Jregistered,[atpath{2}(1:sind(end)) '\'  num2str(Z)  'nm_registered_invivo2Tissue.tif'])

   case 5 %
       % save(['D:\DATA_Glab\Light_sectioning\' mouse '\fit_files\' M_num '_proceesed_1100nm_' num2str(Z) '_tform_' method_fixed ],'mytform')
        save(['D:\DATA_Glab\Light_sectioning\' mouse '\fit_files\' M_num '_proceesed_1100nm_' num2str(Z) '_FIT_' method_fixed ],'FIT')
        imwrite(Jregistered,[atpath{2} 'section_registered_' num2str(Z) '_2.tif'])
